...
export type SchemaMeta = Record<string, {type: s.Type}>;
...
export type StorageData<T extends BaseModel> = {[key in keyof T]?: any};
...
export abstract class BaseModel {
  ...
  public static _fromRawData<T extends BaseModel>(
    ...
    return fromStorage(this, data, options);
  }
  ...
  public static _toRawData<T extends BaseModel>(
    ...
    return modelToStorage(this, m);
  }
}

export function fromStorage<T extends BaseModel>(
...
export function modelToStorage<T extends BaseModel>(
  modelClass: ModelClass<T>,
  m: Partial<T>,
): StorageData<T> {
  const schema = ModelUtil.getSchema(modelClass);
  ...
  for (let key in schema) {
    const shadowKey = '_' + key;
    // @ts-ignore
    if (shadowKey in m)
      // @ts-ignore
      data[key] = schema[key].type.toRawData(m[shadowKey]);
    // @ts-ignore
    else if (key in m) {
