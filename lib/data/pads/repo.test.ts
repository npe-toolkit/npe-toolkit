...
beforeAll(async () => {
  ...
  await mongo.initDb(uri, 'test');
  // console.log('Init test db. mongo uri', uri);
  aRepo = new mongo.RepoMongo(AModel);
  bRepo = new mongo.RepoMongo(BModel);
  cRepo = new mongo.RepoMongo(CModel);
});
...
describe('edges', () => {
  ...
  test('walk inverse edges depth 0/1/2', async () => {
    ...
    const xRepo = new mongo.RepoMongo(XModel);
    const yRepo = new mongo.RepoMongo(YModel);
    const zRepo = new mongo.RepoMongo(ZModel);
...
describe('transaction', () => {
  test('single repo txn - sucess', async () => {
    let a1;
    await mongo.RepoMongo.runWithTransaction(async transaction => {
  ...
  test('multi repos txn - success', async () => {
    let a1;
    let b1 = await mongo.RepoMongo.runWithTransaction(async transaction => {
      const aRepoTxn = aRepo.useTransaction(transaction);
      a1 = new AModel();
      a1.n = 1;
      a1.s = 'a1';
  ...
  test('multi repos txn - fail', async () => {
    ...
    let b1;
    await expect(async () => {
      b1 = await mongo.RepoMongo.runWithTransaction(async transaction => {
        const aRepoTxn = aRepo.useTransaction(transaction);
