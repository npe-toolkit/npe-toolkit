...
import {storageToFirebase} from '@toolkit/providers/firebase/DataStore';
import {
...
const triggerPath =
  getFirestorePathPrefix(instance) + '{collectionId}/{documentId}';
...
export const dryrunDeletion = registerHandler(
  ...
  async input => {
    ...
    return Object.values(dryrunDELETEDStore).map((obj: DELETED) =>
      // @ts-ignore
      storageToFirebase(obj.modelData),
    );
  },
  {
    allowedRoles: ['admin', 'dev'],
    ...
    dryrunRegistry.registryOfChoice
      ...
      .forEach((store: Record<string, BaseModel>, modelName: string) => {
        ...
        Object.values(store).forEach(obj =>
          results.push(storageToFirebase(obj)),
        );
      });
